...
onMounted(() => {
  settingApi.ground.get().then(response => {
    roomGroundForm.value.groundSetting = response.data
  })
})
...
const handleNext = () => {
  roomGroundFormRef.value.validate(async valid => {
    if (valid) {
      try {
        luaparse.parse(roomGroundForm.value.groundSetting);
        await settingApi.ground.post(roomGroundForm.value)
        emit('update:step', props.step + 1)
      } catch (e) {
