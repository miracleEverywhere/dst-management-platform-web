export default {
  ...
  setting: {
    ...
    multiWorldTip: '如果世界个数大于2，请点击右侧按钮加入多层世界模组；如世界个数小于等于2，则无需修改',
    multiWorldDialog: {
    ...
    import: {
      ...
      text1_1: '建议导入完成后，不要立即启动游戏，前往设置-模组页面，下载所有使用的模组，并禁用后重新启用',
  ...
  tools: {
    install: {
      ...
      checkPassDesc1: '检测到游戏已安装，点击下方按钮可重新安装',
      checkNotPass: '检查不通过',
    ...
    backup: {
      ...
      ImportBackup: '导入备份',
      ImportBackupTip: '请导入由饥荒管理平台创建的备份，并且集群名要相同，否则会恢复失败(不推荐使用此功能，建议解压后重新打包并导入)',
      ImportDialog: {
        title: '上传备份文件',
        text1: '将文件拖放到此处',
        text2: '或点击上传',
        tip: '请上传由饥荒管理平台生成的.tgz备份文件，并且集群名必须一致',
      },
      deleteTip: '请至少选择一个文件',
      timeChangeTip: '请选择时间',
